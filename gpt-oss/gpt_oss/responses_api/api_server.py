...
def create_api_server(
    ...
    app = FastAPI()
    # Allow cross-origin calls (useful for dev tunnels or direct browser access)
    try:
        app.add_middleware(
            CORSMiddleware,
            allow_origins=["*"],
    ...
    def root():
        return {
            "service": "GPT-OSS Responses API",
            "status": "ok",
            "message": "This is the backend API. Use POST /v1/responses for inference.",
            "docs": "/docs",
            "frontend_hint": "Launch the Gradio UI at gpt-oss-web/fixed_app.py (default http://127.0.0.1:7862)",
        }
